(function($, Granite) {
    ...
    var titleChatGptToken = 'coral-numberinput[name="./titleToken"]';
    ...
    function retrievePageInfo(dialogContent) {
        ...
        var urlCall = "/bin/teaserPromoServlet" + "?pagePath=" + url + "&titleToken=" + $(titleChatGptToken).val() + "&descToken=" + $(descriptionChatGptToken).val();
        if (url && url.startsWith("/")) {
            return $.ajax({
                ...
                if (data) {
                    ...
                    titleTuple.seedTextValue(JSON.parse(data['jcr:title']).choices[0].text);
                    titleTuple.update();
                    descriptionTuple.seedTextValue(JSON.parse(data['jcr:description']).choices[0].text);
